...
/// JSON-backed ring buffer (~24h) for BPM samples while the app runs foreground sampling.
...
/// BPM from `OTHeartRateMonitoring resolvedHeartRate...` (same semantics as waypoint publish).
...
/// Registers UIApplication / HR preference / BLE notifications once (call from app delegate at launch).
